...
**Статус на момент передачи:** базовая сборка старого Android была запущена и **не завершилась**
(первый прогон долгий: Gradle тянет зависимости, кэш `~/.gradle` уже 909 МБ).
Дошло до `:app:dataBindingMergeDependencyArtifactsDebug` без ошибок. APK ещё не получен.
**Первым делом добейся успешной базовой сборки** — она подтверждает, что тулчейн рабочий,
до того как менять код.
...
### Шаг 0. Убедиться, что база собирается
Догнать запущенную сборку до конца. Если упала — чинить тулчейн, **не трогая код**.
...
Начни с шага 0 и доложи результат базовой сборки, прежде чем менять код.
