...
//! шлёт зашифрованный текст на btest2 через канонический wire-протокол.
//! Запуск (нативно, на Mac): cargo run --example send_test -- "текст"
//! Проверяет, что iOS-приложение (залогинено как btest2) расшифрует и покажет сообщение.
...
fn main() {
    ...
    let recipient = std::env::args().nth(2).unwrap_or_else(|| "btest2".to_string());

    let api = ApiClient::new(base.to_string());
    let kp = generate_keypair();
    let uid = format!("xtest_{}", &sm_core::crypto::random_key_b64()[..8].to_lowercase());
    let enc_priv = sm_core::crypto::encrypt_private_key(kp.private_b64.clone(), "pass1234".into()).unwrap();

    let session = api
        .register(uid.clone(), "pass1234".into(), kp.public_b64.clone(), enc_priv)
        .expect("register");
