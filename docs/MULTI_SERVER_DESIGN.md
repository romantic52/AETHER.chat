...
Бутстрап первого OWNER. Здесь есть ловушка, которую видно только при
реализации: дефолтный режим нового инстанса — `APPROVAL`, то есть
зарегистрироваться не может НИКТО, а одобрять заявки некому. Замкнутый круг.

Разрывается **одноразовым кодом владельца**:

* при старте сервер проверяет, есть ли хоть один OWNER. Нет — генерирует код
  вида `G3K9-7E7H-LYNF`, кладёт в `server_meta` его sha256 и печатает сам код
  в свой лог заметным блоком;
* тот, кто поднял VPS, видит код в `journalctl`/`docker logs` и вводит его в
  приложении как код приглашения. Аккаунт создаётся сразу с ролью `OWNER`
  независимо от режима регистрации;
* код гасится в той же транзакции, в которой создаётся пользователь. Второй
  раз он не сработает;
* интернету код неизвестен, поэтому «кто первым нашёл свежий сервер, тот и
  владелец» не случается.

Альтернатива для автоматизированной установки — `AETHER_OWNER_USER=<username>`
в окружении: если таблица ролей пуста, а такой пользователь уже есть, он
получает OWNER при старте.
