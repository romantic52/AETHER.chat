...
1. научить ядро ЧИТАТЬ оба формата, продолжая писать старый — выпустить;
2. дождаться, пока обновятся Android и веб;
3. переключить запись на Argon2id и пересчитывать блоб при входе, как это
   уже делается с серверным хешем.
